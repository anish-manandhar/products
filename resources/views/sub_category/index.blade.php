...
@section('content')
    <div class="container-fluid p-3">
        <h3 class="text-black-50 text-center">Sub Category</h3>
        <hr>
                ...
                <table class="table table-hover text-center" id="myTable">
                    ...
                    <tbody>
                        @forelse ($subcategory as $data)
                            <tr>
                                <th scope="row">{{ $subcategory->firstItem() + $loop->index }}</td>
                                <td>{{ $data->category->name }}</td>
                                <td>{{ $data->name }}</td>
                                <td>{{ $data->description }}</td>
                                <td class="d-flex justify-content-between align-items-center">
                                    <a href="{{ route('subcategory.edit', $data->id) }}" class="btn btn-primary"><i
                        ...
                        @empty
                            <tr>
                                <h5 class="text-center text-danger">No sub categories to show </h5>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
